...
- **`osu-wiki` directory path** - local path to your osu-wiki fork
...
If you want to run commands through a separate development config, use `pnpm loved:dev <command> [options]`, which will use `config/config.dev.json` instead of `config/config.json`.
