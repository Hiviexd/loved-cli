 ...
 */
function getExtraBeatmapsetInfo(nomination: Nomination): string {
    ...
    if (beatmaps.length > 5) {
        ...
        info += beatmaps
            .map(
                (beatmap) =>
                    (beatmap.key_mode == null ? "" : `[${beatmap.key_mode}K] `) + `${beatmap.star_rating.toFixed(2)}★`
            )
            .join(", ");
    }
 ...
 */
function packUrl(roundId: number, roundTitle: string, ruleset: Ruleset): string {
...
async function generateNews(
    ...
    for (const gameMode of Ruleset.all()) {
        ...
        const nominationsForMode = roundInfo.allNominations.filter((n) => n.game_mode.id === gameMode.id);
        packUrls[gameMode.id] = encodeURI(packUrl(roundId, roundInfo.title, gameMode));
...
export const newsCommand = new Command("news")
    ...
    .action(async (options) => {
        ...
        if (options.threads) {
            ...
            await generateTopics(lovedWeb, osuApi, roundInfo, roundId, options.dryRun).catch(logAndExit);
        }
